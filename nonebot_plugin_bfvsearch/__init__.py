...
async def get_banlog(session: aiohttp.ClientSession, persona_id: str) -> Optional[Dict[str, Any]]:
    url_all = f'https://api.bfvrobot.net/api/v2/bfv/getBannedLogsByPid?personaId={persona_id}'
    data = await fetch_json(session, url_all)
    return data
...
async def server_search_handel(args: Message = CommandArg()):
    ...
    async with aiohttp.ClientSession() as session:
        ...
        if servers_data and 'data' in servers_data:
            ...
            html = f"""
            <h1 style="text-align: center; font-size: 20px;">搜索到以下服务器包含 {search_name}</h1>
            ...
            for server in servers:
                ...
                # 为每个表格设定固定大小，并排放两个表格
                html += f"""
                <div style="width: 48%; margin-bottom: 20px; border: 1px solid #ddd; border-radius: 10px; overflow: hidden; box-shadow: 0 0 10px rgba(0, 0, 0, 0.1);">
                    <table style="width: 100%; height: 200px; border-collapse: collapse;">
                        <tr>
                            <td rowspan="3" style="text-align: center; width: 30%;">
                                <img src="{serverUrl}" style="width: 100%; height: auto; max-height: 180px; border-radius: 8px;" />
                            </td>
                            <td style="padding: 10px; text-align: left; font-weight: bold;">{serverName}</td>
                        </tr>
                        <tr>
                            <td style="padding: 10px; text-align: left;">地图: {serverMap}</td>
                        </tr>
                        <tr>
                            <td style="padding: 10px; text-align: left;">人数: {serverInfo}</td>
                        </tr>
                    </table>
                </div>
